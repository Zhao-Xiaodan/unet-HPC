...
#PBS -j oe
#PBS -k oed
#PBS -N Density_UNet_AutoContrast
...
#   5. ✅ 3-panel tile visualizations with AUTO-CONTRAST (5 representative tiles per image):
#      - Panel 1: Original tile
#      - Panel 2: Predicted mask with AUTO-CONTRAST (white beads)
#      - Panel 3: CLAHE+Otsu (white beads, denoised)
#
# Model Selection:
...
echo "========================================================================"
echo "UNET-ONLY DENSITY ANALYSIS - AUTO-CONTRAST ENHANCEMENT"
echo "========================================================================"
...
echo "========================================================================"
echo "RUNNING DENSITY ANALYSIS WITH AUTO-CONTRAST"
echo "========================================================================"
echo "Command: singularity exec --nv \"$image\" python3 $SCRIPT"
echo ""

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
singularity exec --nv "$image" python3 "$SCRIPT" 2>&1 | tee "density_analysis_unet_only_autocontrast_console_${TIMESTAMP}.log"
...
if [ $EXIT_CODE -eq 0 ]; then
    OUTPUT_DIR=$(find . -maxdepth 1 -type d -name "density_analysis_unet_only_autocontrast_*" | sort | tail -1)

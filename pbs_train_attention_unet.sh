...
#PBS -j oe
#PBS -k oed
#PBS -N AttentionUNet_Hyperparam
#PBS -l select=1:ncpus=36:mpiprocs=1:ompthreads=36:ngpus=1:mem=240gb
#PBS -M phyzxi@nus.edu.sg
#PBS -m abe

################################################################################
# Attention UNet Hyperparameter Search
################################################################################
#
# Purpose: Train Attention UNet with hyperparameter tuning
#
# Features:
...
#
# Model: Attention UNet (with attention gates)
...
echo "========================================================================"
echo "ATTENTION UNET HYPERPARAMETER SEARCH"
echo "========================================================================"
...
SCRIPT="./train_attention_unet_hyperparam.py"
...
singularity exec --nv "$image" python3 "$SCRIPT" 2>&1 | tee "train_attention_unet_hyperparam_console_${TIMESTAMP}.log"
...
if [ $EXIT_CODE -eq 0 ]; then
    OUTPUT_DIR=$(find . -maxdepth 1 -type d -name "attention_unet_hyperparam_*" | sort | tail -1)
    ...
    if [ -n "$OUTPUT_DIR" ]; then
        ...
        if [ -f "$OUTPUT_DIR/attention_unet_results.csv" ]; then
            echo "Results Summary:"
            head -20 "$OUTPUT_DIR/attention_unet_results.csv"
            echo ""
        fi

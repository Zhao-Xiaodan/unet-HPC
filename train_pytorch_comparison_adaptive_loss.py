...
This script uses the SAME setup as train.py but with architecture grid search:
1. Same preprocessing as train.py (grayscale, percentile normalization)
2. Same dataset split (80/20 from ./dataset_shrunk_masks)
3. Same loss function as train.py (AdaptiveBGDiceLoss)
4. Same augmentation as train.py (synthetic background artifacts)
...
- Grid search over architectures and hyperparameters
- Direct comparison of architectures with advanced loss function
...
CONFIG = {
    ...
    # Output
    'output_dir_base': './pytorch_comparison_adaptive_loss',
    ...
    # Loss function (AdaptiveBGDiceLoss - same as train.py)
    'loss_type': 'focal',
    'focal_gamma': 2.0,
    'focal_alpha': 0.4,  # train.py uses 0.4, not 0.25
    'tv_weight': 1.0,
    'dice_weight': 0.4,
    'illum_kernel': 193,
    'delta': 0.05,
    'bg_adapt_weight': 0.05,
    'emphasize': 'fg',
    'b_norm_mode': 'none',
    'tv_flatness_beta': 10.0,
...
def box_blur(x: torch.Tensor, k: int = 63):
    """Box blur using avg pooling (same as train.py)"""
    k = int(k) | 1
    pad = k // 2
...
class FocalBCEWithLogitsLoss(nn.Module):
    """Focal BCE loss (same as train.py)"""
...
class AdaptiveBGDiceLoss(nn.Module):
    """
    Adaptive Background Dice Loss (same as train.py)

    Multi-component loss:
    - Main: Focal BCE
    - BG adaptive: Penalizes over-prediction on bright backgrounds
    - TV: Total variation regularization on flat backgrounds
    - Tversky: Asymmetric Dice loss
    """
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, scaler):
    ...
    for images, masks in dataloader:
        ...
        with autocast(enabled=True):
            ...
            # AdaptiveBGDiceLoss returns (total_loss, logs)
            loss, logs = criterion(outputs, masks, images)
...
def train_model(arch_name, n_filters, dropout, learning_rate, config, train_loader, val_loader, device):
    ...
    # Loss and optimizer (AdaptiveBGDiceLoss - same as train.py)
    criterion = AdaptiveBGDiceLoss(
        ...
        illum_kernel=config['illum_kernel'],
        delta=config['delta'],
        bg_adapt_weight=config['bg_adapt_weight'],
        emphasize=config['emphasize'],
        b_norm_mode=config['b_norm_mode'],
        tv_flatness_beta=config['tv_flatness_beta']
    )

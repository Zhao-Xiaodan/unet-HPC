...
Applies optimization-based visualization to Attention ResU-Net architecture to understand:
- What patterns activate specific channels
- How residual connections affect feature learning
- How attention gates modulate feature representations
- Differences between standard U-Net, Attention U-Net, and Attention ResU-Net
...
# ============================================================================
# ATTENTION RESU-NET MODEL DEFINITION
# ============================================================================
...
class ResConvBlock(nn.Module):
    ...
    Implements residual learning from "Deep Residual Learning for Image Recognition"
    (He et al., 2015). Residual connections help:
    - Alleviate vanishing gradients in deep networks
    - Enable training of much deeper architectures
    - Improve feature propagation and reuse
    """
...
class AttentionGate(nn.Module):
    ...
    Implements spatial attention mechanism from "Attention U-Net:
    Learning Where to Look for the Pancreas" (Oktay et al., 2018)

    The attention gate learns to focus on salient features while suppressing
    irrelevant regions based on both:
    - Gating signal from decoder (coarse, high-level features)
    - Skip connection from encoder (fine, low-level features)
    """
    ...
    def forward(self, g, x):
        ...
        Args:
            ...
            x: skip connection from encoder [B, F_l, H*2, W*2]
        Returns:
            attention-modulated features [B, F_l, H*2, W*2]
        """
        g1 = self.W_g(g)
        x1 = self.W_x(x)
...
class AttentionResUNet(nn.Module):
    ...
    def forward(self, x, return_intermediates=False):
        ...
        if return_intermediates:
            intermediates = {
                'encoder_1_resconv': e1,
                'encoder_2_resconv': e2,
                'encoder_3_resconv': e3,
                'encoder_4_resconv': e4,
                'bottleneck_resconv': b,
                'attention_gate_4': e4_att,  # Attention-modulated features
                'attention_gate_3': e3_att,
                'attention_gate_2': e2_att,
                'attention_gate_1': e1_att,
                'decoder_4_resconv': d4,
                'decoder_3_resconv': d3,
                'decoder_2_resconv': d2,
                'decoder_1_resconv': d1,
            }
...
class FeatureVisualizer:
    """
    Optimization-based feature visualization for Attention ResU-Net
    ...
    def __init__(self, model, device='cuda'):
        ...
        # Set BatchNorm to train mode to prevent NaN issues
    ...
    def visualize_channel(
        ...
        Args:
            layer_name: Name of layer to visualize (e.g., 'decoder_1_resconv', 'attention_gate_3')
...
def main():
    parser = argparse.ArgumentParser(
        description='Feature Visualization for Attention ResU-Net'
    )
    ...
    parser.add_argument('--layers', type=str, nargs='+',
                       default=['encoder_1_resconv', 'encoder_3_resconv',
                               'bottleneck_resconv', 'attention_gate_3',
                               'decoder_1_resconv'],
    ...
    print(f"\n{'='*70}")
    print(f"Attention ResU-Net Feature Visualization")
    print(f"{'='*70}")
    ...
    model = AttentionResUNet(in_channels=1, n_filters=args.n_filters, dropout=args.dropout)
    ...
    print(f"\nKey features:")
    print(f"  - Residual connections: Compare with standard U-Net")
    print(f"  - Attention gates: See what patterns attention focuses on")
    print(f"  - Combined benefits: Residuals + Attention = Best of both worlds")

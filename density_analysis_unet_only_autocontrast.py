...
CONFIG = {
    ...
    'output_dir': f'./density_analysis_unet_only_autocontrast_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
...
# ============================================================================
# VISUALIZATION - REPRESENTATIVE TILES (3-PANEL WITH AUTO-CONTRAST)
# ============================================================================
...
def create_representative_tile_visualizations(tile_data, output_dir, config):
    ...
    - Panel 2: Inverted Predicted mask with AUTO-CONTRAST (white beads on dark background)
    - Panel 3: Inverted CLAHE+Otsu mask (white beads on dark background)

    KEY UPDATE: Panel 2 now uses AUTO-CONTRAST (vmin/vmax from actual data range)
    """
    print("\nGenerating representative tile visualizations (3-panel with AUTO-CONTRAST)...")

    output_dir = Path(output_dir)
    tiles_dir = output_dir / 'representative_tiles_3panel_autocontrast'
    ...
    for image_name, tiles in tiles_by_image.items():
        ...
        for row_idx, tile_info in enumerate(representative_tiles):
            ...
            # AUTO-CONTRAST: Calculate vmin/vmax from actual data range
            vmin_actual = inverted_pred.min()
            vmax_actual = inverted_pred.max()

            axes[row_idx, 1].imshow(inverted_pred, cmap='gray', vmin=vmin_actual, vmax=vmax_actual)
            axes[row_idx, 1].set_title(f'Predicted (AUTO-CONTRAST)\n(White = Beads)\nDensity: {density_threshold_05:.4f}\nRange: [{vmin_actual:.3f}, {vmax_actual:.3f}]',
                                       fontsize=10)
        ...
        # Add overall title
        fig.suptitle(f'{image_name} - Representative Tiles (5 tiles, AUTO-CONTRAST)',
                    fontsize=16, fontweight='bold', y=0.995)

        plt.tight_layout(rect=[0, 0, 1, 0.995])

        # Save
        output_filename = f'tiles_3panel_autocontrast_{dilution_label}_{image_name.replace(".tif", "").replace(".tiff", "")}.png'
...
def save_results(df_tile_results, output_dir, best_model_info, config):
    ...
    experiment_info = {
        ...
        'script': 'density_analysis_unet_only_autocontrast.py',
        'visualization_enhancement': 'AUTO-CONTRAST for Panel 2 (vmin/vmax from actual data range)',

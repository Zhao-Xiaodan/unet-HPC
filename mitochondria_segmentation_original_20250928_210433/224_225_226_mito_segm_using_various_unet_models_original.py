...
# Create output directory with timestamp for this original test
output_dir = f'mitochondria_segmentation_original_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
...
from models_original import Attention_ResUNet, UNet, Attention_UNet, dice_coef, dice_coef_loss, jacard_coef
...
unet_model.save(f'{output_dir}/mitochondria_UNet_50epochs_B_focal_original.hdf5')
...
att_unet_model.save(f'{output_dir}/mitochondria_Attention_UNet_50epochs_B_focal_original.hdf5')
...
att_res_unet_model.save(f'{output_dir}/mitochondria_AttResUnet_50epochs_B_focal_original.hdf5')
...
with open(f'{output_dir}/unet_history_df_original.csv', mode='w') as f:
    unet_history_df.to_csv(f)

with open(f'{output_dir}/att_unet_history_df_original.csv', mode='w') as f:
    att_unet_history_df.to_csv(f)

with open(f'{output_dir}/custom_code_att_res_unet_history_df_original.csv', mode='w') as f:
...
results_summary = {
    'UNet_execution_time': str(execution_time_Unet),
    'Attention_UNet_execution_time': str(execution_time_Att_Unet),
    'AttentionRes_UNet_execution_time': str(execution_time_AttResUnet),
    'UNet_best_val_jaccard': max(unet_history.history['val_jacard_coef']),
    'Attention_UNet_best_val_jaccard': max(att_unet_history.history['val_jacard_coef']),
    'AttentionRes_UNet_best_val_jaccard': max(att_res_unet_history.history['val_jacard_coef']),
    ...
    'implementation': 'ORIGINAL_BROKEN_JACCARD'
}

import json
with open(f'{output_dir}/original_results_summary.json', 'w') as f:
    json.dump(results_summary, f, indent=2)

print(f"\n🚨 ORIGINAL BROKEN IMPLEMENTATION COMPLETE!")
print(f"📁 Results saved to: {output_dir}")
print(f"⚠️  This uses the BROKEN Jaccard implementation for validation")
print(f"📊 Dataset size: {len(image_dataset)} patches")
print(f"🎯 Best validation Jaccard scores:")
print(f"   UNet: {results_summary['UNet_best_val_jaccard']:.6f}")
print(f"   Attention UNet: {results_summary['Attention_UNet_best_val_jaccard']:.6f}")
print(f"   Attention ResUNet: {results_summary['AttentionRes_UNet_best_val_jaccard']:.6f}")

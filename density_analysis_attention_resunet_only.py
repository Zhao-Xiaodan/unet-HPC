#!/usr/bin/env python3
"""
Attention ResUNet-Only Density Analysis with Improved Visualizations
=====================================================================

Script: density_analysis_attention_resunet_only.py
PBS Script: pbs_density_analysis_attention_resunet_only.sh

Performs density analysis using the BEST Attention ResUNet model from hyperparameter search.
...
Uses the BEST Attention ResUNet model from hyperparameter search:
- Source: attention_resunet_hyperparam_20251015_235542/checkpoints/
...
from models_fixed import RepeatElements  # Custom layer for Attention ResUNet
...
CONFIG = {
    # Directories
    'model_base_dir': './attention_resunet_hyperparam_20251015_235542',
    'test_images_dir': './test_images',
    'output_dir': f'./density_analysis_attention_resunet_only_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
...
def print_header(config, best_model_info):
    ...
    print("="*80)
    print("ATTENTION RESUNET-ONLY DENSITY ANALYSIS - BEST MODEL FROM HYPERPARAMETER SEARCH")
    print("="*80)
    print(f"Script: density_analysis_attention_resunet_only.py")
    print(f"PBS Script: pbs_density_analysis_attention_resunet_only.sh")
...
def find_best_attention_resunet_model(base_dir):
    """
    Find the best Attention ResUNet model from hyperparameter search.
    Looks through all models and finds the one with highest validation IoU.
    """
    print("Searching for best Attention ResUNet model...")
    ...
    if not model_dirs:
        raise FileNotFoundError(f"No Attention ResUNet model directories found in {base_dir / 'checkpoints'}")

    print(f"Found {len(model_dirs)} Attention ResUNet model configurations")
    ...
    for model_dir in model_dirs:
        ...
        # Format: attention_resunet_n_filters16_dropout0p1_batch_normTrue_learning_rate0p001
    ...
    if best_model_info is None:
        raise FileNotFoundError("Could not find any valid Attention ResUNet models with validation IoU data")
...
def main():
    ...
    # Find best model
    best_model_info = find_best_attention_resunet_model(CONFIG['model_base_dir'])

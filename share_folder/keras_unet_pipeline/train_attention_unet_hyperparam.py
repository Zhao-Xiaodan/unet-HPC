#!/usr/bin/env python3
"""
Attention UNet Training with Hyperparameter Search
===================================================

Trains Attention UNet with hyperparameter tuning.
...
CONFIG = {
    ...
    # Output
    'output_dir': f'./attention_unet_hyperparam_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
...
def print_header(config):
    ...
    print("="*80)
    print("ATTENTION UNET HYPERPARAMETER SEARCH")
    print("="*80)
    print(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Output directory: {config['output_dir']}")
    print(f"Model: Attention UNet")
...
def run_hyperparam_search(X_train, y_train, X_val, y_val, output_dir, config):
    """
    Run hyperparameter search for Attention UNet.

    Returns:
        DataFrame with results for all hyperparameter combinations
    """
    print("="*80)
    print(f"HYPERPARAMETER SEARCH: ATTENTION UNET")
    print("="*80)
    print()
    ...
    for idx, hyperparams in enumerate(hyperparam_combos):
        ...
        experiment_name = f"attention_unet_" + "_".join([f"{k}{v}" for k, v in hyperparams.items()])
...
def main():
    ...
    # Save results
    results_path = output_dir / 'attention_unet_results.csv'
    ...
    if 'best_val_iou' in results_df.columns:
        ...
        if len(results_clean) > 0:
            ...
            print(f"ATTENTION UNET - Best Configuration:")

...
def extract_dilution_factor(filename):
    ...
    # Use regex with word boundary to match dilution factor at start or after delimiter
    # Pattern: dilution number followed by 'x', at start of string or after underscore/dash
    match = re.search(r'(?:^|_)(\d+)x(?:_|\.|-)', filename.lower())
    if match:
        return int(match.group(1))

    # Fallback: try to match at the very beginning

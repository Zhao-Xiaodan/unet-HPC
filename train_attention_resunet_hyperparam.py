#!/usr/bin/env python3
"""
Attention ResUNet Training with Hyperparameter Search
======================================================

Trains Attention ResUNet with hyperparameter tuning.
...
from models_fixed import build_attention_resunet, RepeatElements
...
CONFIG = {
    ...
    # Output
    'output_dir': f'./attention_resunet_hyperparam_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
...
def print_header(config):
    ...
    print(f"Model: Attention ResUNet")
...
def run_hyperparam_search(X_train, y_train, X_val, y_val, output_dir, config):
    """
    Run hyperparameter search for Attention ResUNet.

    Returns:
        DataFrame with results for all hyperparameter combinations
    """
    print("="*80)
    print(f"HYPERPARAMETER SEARCH: ATTENTION RESUNET")
    print("="*80)
    print()
    ...
    for idx, hyperparams in enumerate(hyperparam_combos):
        ...
        experiment_name = f"attention_resunet_" + "_".join([f"{k}{v}" for k, v in hyperparams.items()])
...
def main():
    ...
    # Save results
    results_path = output_dir / 'attention_resunet_results.csv'
    ...
    if 'best_val_iou' in results_df.columns:
        ...
        if len(results_clean) > 0:
            ...
            print(f"ATTENTION RESUNET - Best Configuration:")

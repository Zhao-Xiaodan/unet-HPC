#!/usr/bin/env python3
"""
Attention ResU-Net Advanced Feature Visualization
==================================================
...
5. Full feature map exports

Based on visualize_unet_features_advanced.py but adapted for Attention ResU-Net architecture.
...
class AttentionResUNet(nn.Module):
    """Attention ResU-Net with residual blocks and attention gates"""
    def __init__(self, in_channels=1, n_filters=64, dropout=0.1):
        super().__init__()
...
# ============================================================================
# ATTENTION RESU-NET SPECIFIC FUNCTIONS
# ============================================================================

def register_attention_resunet_hooks(model):
    """Register hooks for Attention ResU-Net layers including attention gates"""
    layers_dict = {}

    # Encoder/Decoder residual blocks
    def get_resconv_layers(block, prefix):
        if hasattr(block, 'bn2'):
            layers_dict[f'{prefix}_resconv'] = block.bn2

    get_resconv_layers(model.enc1, 'encoder_1')
    get_resconv_layers(model.enc2, 'encoder_2')
    get_resconv_layers(model.enc3, 'encoder_3')
    get_resconv_layers(model.enc4, 'encoder_4')
    get_resconv_layers(model.bottleneck, 'bottleneck')
    get_resconv_layers(model.dec4, 'decoder_4')
    get_resconv_layers(model.dec3, 'decoder_3')
    get_resconv_layers(model.dec2, 'decoder_2')
    get_resconv_layers(model.dec1, 'decoder_1')
...
def get_attention_resunet_layer_dimensions(n_filters=64):
    """Get spatial dimensions and channel counts for each layer"""
    return {
        'encoder_1_resconv': (512, n_filters),
        'encoder_2_resconv': (256, n_filters * 2),
        'encoder_3_resconv': (128, n_filters * 4),
        'encoder_4_resconv': (64, n_filters * 8),
        'bottleneck_resconv': (32, n_filters * 16),
        'decoder_4_resconv': (64, n_filters * 8),
        'decoder_3_resconv': (128, n_filters * 4),
        'decoder_2_resconv': (256, n_filters * 2),
        'decoder_1_resconv': (512, n_filters),
        # Attention gates
        'attention_gate_4': (64, 1),
...
def visualize_attention_resunet_feature_maps(model, image_tensor, output_dir, device, n_clusters=8):
    """
    Visualize feature maps with PCA/UMAP clustering for Attention ResU-Net
    """
    ...
    print("\nVisualizing Attention ResU-Net feature maps with PCA clustering...")
    ...
    layers_to_visualize = register_attention_resunet_hooks(model)
    hooks = {}
...
def visualize_attention_resunet_inversions(model, image_tensor, output_dir, device, n_filters=64):
    """Feature inversions at correct dimensions for Attention ResU-Net"""
    output_dir = Path(output_dir / 'feature_inversions')
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\nVisualizing Attention ResU-Net feature inversions...")

    layer_dims = get_attention_resunet_layer_dimensions(n_filters)
    all_layers = register_attention_resunet_hooks(model)

    original_img_plot = base_viz.postprocess_tensor(image_tensor)

    # Only invert resconv layers (skip attention gates for inversion)
    resconv_layers = {k: v for k, v in all_layers.items() if 'attention_gate' not in k}

    for layer_name, layer_module in tqdm(resconv_layers.items(), desc="Running feature inversions"):
    ...
    print(f"  ✓ Saved {len(resconv_layers)} feature inversion visualizations")
...
def main():
    parser = argparse.ArgumentParser(description='Attention ResU-Net Advanced Feature Visualization')
    parser.add_argument('--model_path', type=str, default='./best_models_PyTorch/attention_resunet/best_model.pth',
                       help='Path to Attention ResU-Net model checkpoint')
    parser.add_argument('--test_images_dir', type=str, default='./test_images',
                       help='Directory containing test images')
    parser.add_argument('--output_dir', type=str, default=None,
                       help='Output directory (default: attention_resunet_visualization_advanced_<timestamp>)')
    parser.add_argument('--n_filters', type=int, default=64,
                       help='Number of base filters (default: 64, trained model uses 64)')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f'attention_resunet_visualization_advanced_{timestamp}'

    output_base = Path(args.output_dir)
    output_base.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print("ATTENTION RESU-NET ADVANCED FEATURE VISUALIZATION")
    print("="*80)
    ...
    # Load model
    print("\nLoading Attention ResU-Net model...")
    model = AttentionResUNet(in_channels=1, n_filters=args.n_filters, dropout=args.dropout).to(device)
    ...
    for img_idx, img_path in enumerate(image_files, 1):
        ...
        # Feature inversions
        visualize_attention_resunet_inversions(model, tile_tensor, image_output_dir, device, args.n_filters)

        # Feature maps with clustering
        visualize_attention_resunet_feature_maps(model, tile_tensor, image_output_dir, device, args.n_clusters)
